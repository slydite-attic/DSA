...
#### Time and Space Complexity
- **Time Complexity:** $O(N)$. In the worst case (no cycle), we visit each node. With a cycle, the number of steps is proportional to the number of nodes.
- **Space Complexity:** $O(1)$.
...
#### Time and Space Complexity
- **Time Complexity:** $O(N)$. We traverse the list once to find the tail, and then once more with two pointers.
- **Space Complexity:** $O(1)$.

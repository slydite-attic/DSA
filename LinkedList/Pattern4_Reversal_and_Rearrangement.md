...
#### Time and Space Complexity
- **Time Complexity:** $O(N)$. We visit each node twice at most.
- **Space Complexity:** $O(N/k)$ for recursion stack. Iterative approach would be $O(1)$.
...
#### Time and Space Complexity
- **Time Complexity:** $O(N)$. We traverse the list to find length and then to the break point.
- **Space Complexity:** $O(1)$.
...
#### Time and Space Complexity
- **Time Complexity:** $O(N)$, as we reverse twice and traverse once.
- **Space Complexity:** $O(1)$ (excluding recursion stack for reverse if recursive method used).

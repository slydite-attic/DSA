...
#### Implementation Overview
Any 'O' connected to the boundary of the board cannot be surrounded. We can find all 'O's on the border and run a DFS to mark all 'O's connected to them with a temporary marker (like 'E'). Then we iterate over the board: flip any remaining 'O' to 'X' (since they are surrounded), and flip 'E' back to 'O' (since they are safe).
...
#### Complexity Analysis
- **Time Complexity:** $O(N \times M)$ because we traverse the border and perform a DFS that visits each cell at most once. The final pass is also $O(N \times M)$.
- **Space Complexity:** $O(N \times M)$ in the worst case for the DFS recursion stack.
